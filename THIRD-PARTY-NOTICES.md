# Third-party notices

## Material Design Icons

HoomNote includes selected SVG path geometry from
[Material Design Icons](https://github.com/Templarian/MaterialDesign).
The paths are vendored as local XAML resources; no Material.Icons runtime
package or icon font is distributed.

Material Design Icons is maintained by Pictogrammers and licensed under the
[Apache License 2.0](https://github.com/Templarian/MaterialDesign/blob/master/LICENSE).

